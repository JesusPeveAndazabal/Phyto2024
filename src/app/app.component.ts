      ...
      setInterval(()=>{
        if(!onExecution){
          ...
          const iteration = async () => {
            try {
                const records = await this.databaseService.getNotSendedExecution();
                for (const wExecution of records) {
                    try {
                        let response = wExecution;
                        if (!wExecution.id_from_server) {
                            if (wExecution.configuration.trim() !== '') {
                                wExecution.configuration = JSON.parse(wExecution.configuration);
                                console.log("La cadena está llena");
                            } else {
